...
- In addition, we are considering the night sky background only for new Moon conditions assuming a PSF of 1 arcsec. This limitation might or might not be removed in future versions.
...
# ╔═╡ 5097b23c-fa8c-479a-8a42-3591facac02d
WidthOverDocs()
...
Markdown.parse("""
##### The target input polarization is: $(latexify(SourcePol*100,fmt="%.3f"))% and the position angle θ=$(PosAngle)°
""")
...
begin
	...
	λλ = collect(range(start=300u"nm", step=1u"nm", stop=900u"nm"))
	...
	lines!([λ.val for λ in λλ],Float64.(gfnt.(λλ)),label="g",color=:green)
	...
	lines!([λ.val for λ in λλ],Float64.(rfnt.(λλ)),label="r",color=:red)
	
	axislegend(ax2fg3)

	ax3fg3 = Axis(fg3[2, 1],
	    xlabel = "λ (nm)",
	    ylabel = "Extinction/airmass (mag)"
	    )
	
	lines!([λ.val for λ in λλ],Float64.(extfnt.(λλ)),label="Extinction",color=:blue)
	...
	lines!([λ.val for λ in λλ],Float64.(pfnt.(λλ)),label="Polaroid",color=:brown)
	
	axislegend(ax4fg3,position=:cb)
...
Nigh sky magnitude: $(latexify(skymag)) \\
...
Number of sky photons: $(latexify(Numγsky,fmt=FancyNumberFormatter(3))) \\
...
# ╔═╡ 8a705980-7a75-4542-afcc-a0645c0c430b
#=╠═╡
begin
	dP = sqrt.(sQN.^2 .+ sUN.^2)
...
##### And, finally, the measured polarisation is: $(latexify(PresP,fmt="%.3f")) ± $(latexify(PresErr,fmt="%.3f"))% with a S/N of $(latexify(SN,fmt="%.2f")),

##### while the measured position angle is: $(latexify(θd,fmt="%.2f")) ± $(latexify(eθd,fmt="%.2f"))°.

###### In case it is of interest, the 95% and 99% upper limits for the polarisation degree are: $(latexify(pquant[1],fmt="%.2f"))% and $(latexify(pquant[2],fmt="%.2f"))%, respectively.
...
manifest_format = "2.1"
project_hash = "0906f567de4e25c1ce8b23e5fbc14acf451b43d1"
...
git-tree-sha1 = "3260db49a2599ed83d649b2c3043f1733c3d2895"
registries = "General"
