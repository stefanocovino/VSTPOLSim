...
This polarimetric simulation is based on three-angle demodulation scheme, i.e., I assume to obtain three frames with a rotation of the polaroid filter in between. 

In addition, I am onsidering statistical errors only, and I am also assunming that the background is negligible compared to the target of interest.
"""
...
Total efficiency: $(latexify(ReflEff,fmt="%.2f"))%
""")

# ╔═╡ d415e8cc-7115-48aa-8029-6f08411055e0
md"### Choose the total (3 $\times$ the single exposure) exposure time:"
...
md"### Choose the target magnitude (AB):"
...
md"### Choose linear polarization degree and position angle;"
...
md"""
We also assume to work observing $ModSchema times rotating the polaroid filter by:
$ang1, $ang2 and $(ang3)°, respectively.
"""
...
Let's now compute the number of photons per unit area and time generated by the source in each frame. I am assuming the total efficiency already takes into account the atmospheric absorption and that the sky background is negligible.
"""
...
md"Exposure time for each frame: $effTime"

# ╔═╡ a6c70d57-88ed-41f2-aff2-69f2d0f12868
md"Now I compute the total number of photons from the source at the telescope entrance:"
...
# ╔═╡ 35f11c5b-34ba-4664-94fa-eedbe8094465
# ╠═╡ disabled = true
#=╠═╡
dIs = array_of_empty_vectors(Float64, 3, 1);
  ╠═╡ =#

# ╔═╡ 06d68a82-c7e5-407e-9992-c69e6942c346
# ╠═╡ disabled = true
#=╠═╡
for i in 1:length(Is)
	dIs[i,1] = rand(Poisson(Is[i]),10000)
end
  ╠═╡ =#
...
#=╠═╡
Markdown.parse("""
Measured polarisation: $(latexify(Pres,fmt="%.3f")) ± $(latexify(PresErr,fmt="%.3f"))
""")
  ╠═╡ =#
...
# ╔═╡ 3a857b66-e1bf-4c3b-9b52-299084eef5a4
#=╠═╡
Markdown.parse("""
And the measurement S/N: $(latexify(SN,fmt="%.2f"))
""")
  ╠═╡ =#
