...
# ╔═╡ d8e9f8e3-1106-40e7-aa5c-023572fbf3c1
md"""
#### Number of observing cycles: $( @bind ncycles NumberField(1:1:10, default=1) )
"""
...
Markdown.parse("""
Filter: $(latexify(filter)) \\
Input magnitude: $(latexify(inpmag)) \\
Single frame exposure time: $(latexify(exptime/3)) \\
Single cycle exposure time: $(latexify(exptime)) \\
Total (all cycles) exposure time: $(latexify(totexptime)) \\
Airmass: $(latexify(airm)) \\
Total telescope time (including overheads): $(latexify(tottelesctime)) 
""")
...
md"""
The demodulation schema is based on $ModSchema rotations of the polaroid filter starting at $(ang1)° and then going to $(ang2)° and $(ang3)°.
"""
...
First, let's now compute the number of photons per unit area and time generated by the source in each frame and the actually detected photons by the detector. The total efficiency already takes into account the atmospheric absorption, filters, detector efficiency, etc.
""")

# ╔═╡ 8a370236-b75d-4fdb-a4e5-4a943185a4c9
begin
	Numγ = 0.
	EffNumγ = 0.
	Numγsky = 0.
...
Markdown.parse("""
The single frame total exposure time is: $(latexify(effTime,fmt="%.1f"))
""")

# ╔═╡ 979dc4c8-2c9c-426d-afbc-87996c9ecc0f
Markdown.parse("""
Number of photons from the source: $(latexify(Numγ,fmt=FancyNumberFormatter(3))) \\
Number of detected photons: $(latexify(EffNumγ,fmt=FancyNumberFormatter(3))) 
""")
